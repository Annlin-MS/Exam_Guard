...
const StaffQuestions = () => {
  ...
  const handleDeleteQuestion = async (questionId) => {
    ...
    try {
      ...
      showToast("Question deleted! Add a new one to replace it ✅");
      fetchQuestions(selectedExam.id);
    } catch (err) {
        ...
        <div style={{ flex:1 }}>
          ...
          {showDelete && (
            <button
              ...
              style={{ marginTop:10, padding:"7px 16px", borderRadius:8, border:"1px solid rgba(255,107,107,0.3)", background:"rgba(255,107,107,0.08)", color:"#FF6B6B", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", opacity: deleting===q.id ? 0.6 : 1 }}>
    ...
    <>
      ...
      <div style={{ fontFamily:"'DM Sans',sans-serif", color:"#1a1a2e", maxWidth:1100 }}>
        ...
        <div style={{ marginBottom:24 }}>
          ...
          <div style={{ fontSize:13, color:"rgba(26,26,46,0.45)" }}>Manage questions — submit for admin approval — resubmit after fixes</div>
        </div>
              ...
              exams.map((exam, i) => {
                ...
                return (
                  <div key={exam.id} className="exam-tab fade-up"
                    ...
                    <div style={{ fontSize:11, color:"rgba(26,26,46,0.45)" }}>📅 {exam.exam_date}</div>
              ...
              <>
                ...
                <div className="fade-up" style={{ background:"#fff", border:"1px solid rgba(26,26,46,0.08)", borderRadius:16, padding:20, marginBottom:16 }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12, marginBottom:14 }}>
                    <div>
                      ...
                      <div style={{ fontSize:13, color:"rgba(26,26,46,0.5)", display:"flex", gap:12 }}>
                    ...
                    <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                      ...
                      {selectedExam.workflow_status === "REJECTED" && rejectedQ.length > 0 && (
                        <div style={{ padding:"9px 14px", borderRadius:10, background:"rgba(255,107,107,0.08)", border:"1px solid rgba(255,107,107,0.2)", color:"#FF6B6B", fontSize:12, fontWeight:600 }}>
                          ⚠️ Delete {rejectedQ.length} rejected question(s) first!
                        </div>
                      )}
                  ...
                  {/* Progress Bar */}
                  <div style={{ height:6, background:"rgba(26,26,46,0.06)", borderRadius:3, overflow:"hidden", marginBottom:6 }}>
                  ...
                  <div style={{ fontSize:11, color:"rgba(26,26,46,0.45)" }}>
                    ...
                    {questions.length >= selectedExam.total_questions_allowed && " ✅ Complete"}
                  </div>
                </div>

                {/* Question Stats */}
                {questions.length > 0 && (
                  <div className="fade-up" style={{ display:"flex", gap:10, marginBottom:16, animationDelay:"0.05s" }}>
                    {[
                      ...
                      <div key={i} style={{ padding:"10px 16px", borderRadius:12, background:`${s.color}10`, border:`1px solid ${s.color}22`, textAlign:"center", flex:1 }}>
                ...
                {questionsLoading ? (
                  ...
                  <>
                    ...
                    {/* ❌ REJECTED */}
                    {rejectedQ.length > 0 && (
                      <SectionBlock icon="❌" title="Rejected — Delete & Replace" count={rejectedQ.length} color="#FF6B6B">
                        {/* Warning banner */}
                        {selectedExam.workflow_status === "REJECTED" && (
                          <div style={{ padding:"12px 14px", background:"rgba(255,107,107,0.06)", border:"1px solid rgba(255,107,107,0.2)", borderRadius:10, marginBottom:12, fontSize:13, color:"#FF6B6B", display:"flex", alignItems:"center", gap:8 }}>
                            <span>⚠️</span>
                            <span>Delete rejected questions, add replacements, then resubmit!</span>
                          </div>
                        )}
                        {rejectedQ.map((q,i) => (
                          <QuestionCard
          ...
          <div style={{ background:"#fff", borderRadius:20, padding:28, width:"90%", maxWidth:560, boxShadow:"0 24px 64px rgba(0,0,0,0.15)", maxHeight:"90vh", overflowY:"auto" }}
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14, marginBottom:20 }}>
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                <label style={{ fontSize:12, fontWeight:700, color:"rgba(26,26,46,0.6)", letterSpacing:"0.05em" }}>Question *</label>
              ...
              {["a","b","c","d"].map(opt => (
                <div key={opt} style={{ display:"flex", flexDirection:"column", gap:6 }}>
                  <label style={{ fontSize:12, fontWeight:700, color:"rgba(26,26,46,0.6)", letterSpacing:"0.05em" }}>Option {opt.toUpperCase()} *</label>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                <label style={{ fontSize:12, fontWeight:700, color:"rgba(26,26,46,0.6)", letterSpacing:"0.05em" }}>Correct Answer *</label>
                <div style={{ display:"flex", gap:8 }}>
                  {["A","B","C","D"].map(opt => (
                    <button key={opt} className="act-btn"
