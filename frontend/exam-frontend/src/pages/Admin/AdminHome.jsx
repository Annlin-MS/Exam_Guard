...
const AdminHome = () => {
  ...
  const deptStats = exams.reduce((acc, exam) => {
    const d = exam.department || "Unknown";
    if (!acc[d]) acc[d] = { total:0, locked:0, submitted:0, approved:0, draft:0, students:0 };
    acc[d].total++;
    acc[d].students += exam.enrolled_students_count || 0;
    if (exam.workflow_status === "LOCKED")    acc[d].locked++;
    if (exam.workflow_status === "SUBMITTED") acc[d].submitted++;
    if (exam.workflow_status === "APPROVED")  acc[d].approved++;
    if (exam.workflow_status === "DRAFT")     acc[d].draft++;
    return acc;
  }, {});

  const DEPT_ICONS  = { CS:"💻", ECE:"⚡", MECH:"⚙️", CIVIL:"🏗️", MBA:"📊" };
  const DEPT_COLORS = {
    CS:    { bg:"rgba(108,99,255,0.06)",  border:"rgba(108,99,255,0.18)",  color:"#6C63FF" },
    ECE:   { bg:"rgba(0,201,167,0.06)",   border:"rgba(0,201,167,0.18)",   color:"#00C9A7" },
    MECH:  { bg:"rgba(255,107,107,0.06)", border:"rgba(255,107,107,0.18)", color:"#FF6B6B" },
    CIVIL: { bg:"rgba(255,209,102,0.06)", border:"rgba(255,209,102,0.18)", color:"#FFD166" },
    MBA:   { bg:"rgba(148,163,184,0.06)", border:"rgba(148,163,184,0.18)", color:"#94a3b8" },
  };
  ...
  const ACTION_ICONS = {
    CREATE_EXAM:"📝", APPROVE_EXAM:"✅", LOCK_PAPER:"🔒", SUBMIT_EXAM:"📤",
    PUBLISH_RESULT:"📢", ENROLL_STUDENT:"👨‍🎓", ADD_QUESTION:"❓",
    REJECT_EXAM:"❌", UPDATE_EXAM:"✏️", SUBMIT_APPROVAL:"📋",
  };
  ...
  return (
    <>
      <style>{`
        ...
        .dept-card:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,0,0,0.08) !important; }
        .hash-text { font-family:'Courier New',monospace; font-size:11px; color:#6C63FF; background:#EFE9FF; padding:2px 6px; border-radius:4px; word-break:break-all; }
      `}</style>
      ...
      <div style={s.root}>
          ...
          <div>
            ...
            <div style={s.pageSubtitle}>
              ...
              {blockchain.connected && (
                <span style={{ marginLeft:14, color:"#00C9A7", fontWeight:600, fontSize:12 }}>
                  <span style={{ display:"inline-block", width:7, height:7, borderRadius:"50%", background:"#00C9A7", marginRight:5, animation:"pulse 2s infinite" }} />
                  Blockchain · Block #{blockchain.current_block}
                </span>
              )}
            </div>
          </div>
        ...
        <div style={s.statsRow}>
          {statCards.map((c, i) => (
            <div key={i} className="stat-card"
              style={{ ...s.statCard, border:`1px solid ${c.color}22`, animationDelay:`${i*0.07}s` }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12 }}>
            {actionButtons.map((btn, i) => (
              <a key={i} href={btn.link} className="action-btn"
                style={{ padding:"16px 20px", borderRadius:12, display:"flex", alignItems:"center", gap:12, border:`1px solid ${btn.color}33`, background:"#F7F5FF", textDecoration:"none", transition:"all 0.15s", position:"relative" }}>
                ...
                {btn.label === "Approve Questions" && (stats.pending_approvals ?? 0) > 0 && (
                  <span style={{ position:"absolute", top:-6, right:-6, width:20, height:20, borderRadius:"50%", background:"#FF6B6B", color:"#fff", fontSize:10, fontWeight:800, display:"flex", alignItems:"center", justifyContent:"center" }}>
        ...
        <div style={{ ...s.card, marginBottom:20 }}>
          <div style={s.cardHeader}>
            <span style={s.cardTitle}>🏛️ Department Overview</span>
            <span style={s.cardCount}>{Object.keys(deptStats).length} department(s)</span>
          </div>
          {Object.keys(deptStats).length === 0 ? (
            <div style={s.empty}>No exams created yet</div>
          ) : (
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))", gap:14 }}>
              {Object.entries(deptStats).map(([dept, data]) => {
                const dc   = DEPT_COLORS[dept] || DEPT_COLORS.CS;
                const icon = DEPT_ICONS[dept]  || "🏛️";
                return (
                  <div key={dept} className="dept-card"
                    style={{ padding:"16px 18px", borderRadius:14, background:dc.bg, border:`1px solid ${dc.border}` }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
                      <div style={{ width:38, height:38, borderRadius:10, background:"#fff", display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, boxShadow:`0 2px 8px ${dc.color}22` }}>
                        {icon}
                      </div>
                      <div style={{ fontFamily:"'Syne',sans-serif", fontSize:15, fontWeight:800, color:dc.color }}>{dept}</div>
                    </div>
                    <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                      {[
                        { label:"Total Exams",  value:data.total,     color:dc.color  },
                        { label:"🔒 Locked",    value:data.locked,    color:"#6C63FF" },
                        { label:"⏳ Submitted", value:data.submitted, color:"#FFD166" },
                        { label:"✅ Approved",  value:data.approved,  color:"#00C9A7" },
                        { label:"📝 Draft",     value:data.draft,     color:"#94a3b8" },
                        { label:"👨‍🎓 Students", value:data.students,  color:"#4B2E83" },
                      ].map((row, j) => (
                        <div key={j} style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                          <span style={{ fontSize:11, color:"#8F80C9" }}>{row.label}</span>
            ...
            <div style={{ borderRight:"1px solid #E6E0F8", padding:12, overflowY:"auto", maxHeight:420, background:"#FAFAFA" }}>
              {exams.length === 0 ? (
                <div style={{ textAlign:"center", padding:"40px 10px", color:"#8F80C9", fontSize:12 }}>
                  <div style={{ fontSize:28, marginBottom:8 }}>📭</div>No exams yet
                </div>
              ) : exams.map((exam) => {
                ...
                return (
                  <div key={exam.id} className="exam-tab"
                    ...
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4 }}>
                      <span style={{ fontSize:12, fontWeight:700, color: isSelected ? "#6C63FF" : "#4B2E83" }}>{exam.exam_name}</span>
                      <span style={{ fontSize:9, fontWeight:700, padding:"2px 6px", borderRadius:20, color:sc.color, background:sc.bg }}>{sc.label}</span>
                    </div>
                    <div style={{ fontSize:10, color:"#8F80C9" }}>👨‍🏫 {exam.assigned_staff || "—"} · 📅 {exam.exam_date}</div>
                    <div style={{ fontSize:10, color:"#8F80C9", marginTop:2 }}>🏛️ {exam.department || "—"} {exam.semester ? `· Sem ${exam.semester}` : ""}</div>
                  </div>
                );
              })}
            </div>
              ...
              {!selectedExam ? (
                ...
                <>
                  <div style={{ marginBottom:16, padding:"10px 14px", background:"#F5F2FF", borderRadius:10 }}>
                    <div style={{ fontFamily:"'Syne',sans-serif", fontSize:14, fontWeight:700, color:"#5A3EC8", marginBottom:4 }}>{selectedExam.exam_name}</div>
                  ...
                  {pendingQ.length > 0 && (
                    <button className="act-btn" onClick={() => navigate("/admin/approvals")}
                    ...
                    return (
                      <div key={q.id} className="q-row"
                        ...
                        <div style={{ display:"flex", alignItems:"flex-start", gap:10 }}>
                          <div style={{ width:24, height:24, borderRadius:6, background:`${qColor}15`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:800, color:qColor, flexShrink:0 }}>{i+1}</div>
                          <div style={{ flex:1 }}>
                            <div style={{ fontSize:13, fontWeight:600, color:"#4B2E83", marginBottom:8, lineHeight:1.5 }}>{q.question_text}</div>
                            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:4 }}>
                              {["a","b","c","d"].map(opt => (
                                <div key={opt} style={{ padding:"4px 8px", borderRadius:6, background: q.correct_option===opt.toUpperCase() ? "rgba(0,201,167,0.08)" : "rgba(26,26,46,0.03)", border: q.correct_option===opt.toUpperCase() ? "1px solid rgba(0,201,167,0.2)" : "1px solid rgba(26,26,46,0.06)", display:"flex", alignItems:"center", gap:5 }}>
                                  <span style={{ width:15, height:15, borderRadius:3, background: q.correct_option===opt.toUpperCase() ? "#00C9A7" : "rgba(26,26,46,0.08)", color: q.correct_option===opt.toUpperCase() ? "#fff" : "rgba(26,26,46,0.4)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:8, fontWeight:800, flexShrink:0 }}>{opt.toUpperCase()}</span>
                          ...
                          <span style={{ fontSize:9, fontWeight:700, padding:"3px 8px", borderRadius:20, background:`${qColor}15`, color:qColor, flexShrink:0 }}>{q.status}</span>
                        </div>
                      </div>
                    );
                  })}
        ...
        <div style={s.twoCol}>
          <div style={s.card}>
            ...
            <div style={{ padding:"12px 14px", borderRadius:10, marginBottom:14, background: blockchain.connected ? "rgba(0,201,167,0.06)" : "rgba(255,107,107,0.06)", border:`1px solid ${blockchain.connected ? "rgba(0,201,167,0.2)" : "rgba(255,107,107,0.2)"}`, display:"flex", alignItems:"center", gap:10 }}>
              <span style={{ fontSize:22 }}>{blockchain.connected ? "🟢" : "🔴"}</span>
              <div>
                <div style={{ fontSize:13, fontWeight:700, color: blockchain.connected ? "#00C9A7" : "#FF6B6B" }}>
                  {blockchain.connected ? "Ganache Running" : "Ganache Offline"}
                </div>
                <div style={{ fontSize:11, color:"#8F80C9", marginTop:1 }}>
                  {blockchain.connected ? "Local development blockchain" : "Start Ganache to enable blockchain"}
                </div>
              </div>
            </div>
            <div style={s.bcGrid}>
              <div style={s.bcItem}><div style={s.bcItemLabel}>Network</div><div style={s.bcItemValue}>Ganache Local</div></div>
              <div style={s.bcItem}><div style={s.bcItemLabel}>Current Block</div><div style={{ ...s.bcItemValue, color:"#6C63FF" }}>#{blockchain.current_block ?? "—"}</div></div>
              <div style={s.bcItem}><div style={s.bcItemLabel}>Locked Papers</div><div style={{ ...s.bcItemValue, color:"#00C9A7" }}>{exams.filter(e => e.workflow_status === "LOCKED").length}</div></div>
              <div style={s.bcItem}><div style={s.bcItemLabel}>Transactions</div><div style={{ ...s.bcItemValue, color:"#FFD166" }}>{blockchain.current_block ?? 0}</div></div>
              <div style={{ ...s.bcItem, gridColumn:"1/-1" }}><div style={s.bcItemLabel}>Admin Account</div><div className="hash-text">{blockchain.admin_account ?? "Not connected"}</div></div>
            </div>
            <button onClick={() => navigate("/admin/blockchain")}
              style={{ marginTop:14, width:"100%", padding:"10px", borderRadius:10, border:"1px solid rgba(108,99,255,0.2)", background:"rgba(108,99,255,0.05)", color:"#6C63FF", fontSize:12, fontWeight:700, cursor:"pointer" }}>
              🔗 View Full Blockchain Details →
            </button>
          </div>

          <div style={s.card}>
            <div style={s.cardHeader}><span style={s.cardTitle}>📈 System Overview</span></div>
            <div style={s.overviewList}>
              {[
                { label:"Exams Completed",    value:(stats.total_exams??0)-(stats.active_exams??0),                                                                          color:"#6C63FF" },
                { label:"Approval Rate",       value:`${(stats.total_exams??0)>0 ? Math.round(((stats.total_exams-stats.pending_approvals)/stats.total_exams)*100) : 100}%`, color:"#00C9A7" },
                { label:"Staff:Student Ratio", value:`1:${(stats.total_staff??0)>0 ? Math.round((stats.total_students??0)/(stats.total_staff??1)) : 0}`,                    color:"#FFD166" },
                { label:"Pending Reviews",     value:stats.pending_approvals ?? 0,                                                                                           color:"#FF6584" },
                { label:"Departments Active",  value:Object.keys(deptStats).length,                                                                                          color:"#4B2E83" },
                { label:"Total Enrolled",      value:exams.reduce((sum,e) => sum+(e.enrolled_students_count||0), 0),                                                         color:"#00C9A7" },
              ].map((item, i) => (
            ...
            <div style={s.logList}>
              {logs.slice(0,8).map((log, i) => (
                <div key={i} className="log-item" style={s.logItem}>
                  <div style={{ ...s.logAvatar, fontSize:16 }}>{ACTION_ICONS[log.action] || "⚙️"}</div>
                  <div style={s.logBody}>
                    <div style={s.logDesc}>{log.description}</div>
                    <div style={s.logMeta}>
                      <span style={s.logUser}>👤 {log.user}</span>
                      <span style={s.logTime}>🕐 {timeAgo(log.timestamp)}</span>
                    </div>
                  </div>
                  <div style={{ fontSize:10, fontWeight:700, padding:"3px 8px", borderRadius:20, background:"rgba(108,99,255,0.08)", color:"#6C63FF" }}>
                    {log.action}
                  </div>
                </div>
              ))}
...
const s = {
  ...
  statCard:     { background:"#FFFFFF", borderRadius:14, padding:"20px", boxShadow:"0 4px 12px rgba(108,99,255,0.1)", transition:"all 0.2s" },
  ...
  card:         { background:"#FFFFFF", border:"1px solid #E6E0F8", borderRadius:16, padding:"24px", boxShadow:"0 4px 12px rgba(108,99,255,0.08)" },
  ...
  bcGrid:       { display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 },
  bcItem:       { background:"#F5F2FF", borderRadius:10, padding:"12px 14px" },
  bcItemLabel:  { fontSize:11, color:"#8F80C9", marginBottom:4 },
  bcItemValue:  { fontSize:15, fontWeight:600, color:"#4B2E83" },
  overviewList: { display:"flex", flexDirection:"column", gap:10 },
  overviewItem: { display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 14px", background:"#F5F2FF", borderRadius:10 },
  ...
  logItem:      { display:"flex", alignItems:"center", gap:14, padding:"12px 10px", borderRadius:8, transition:"background 0.15s" },
  ...
  logMeta:      { display:"flex", gap:12, marginTop:2 },
